...
if(!isset($_SESSION['username']) || $_SESSION['usertype'] !='admin'){
...

    {
        ...
        $buyingrate=mysqli_real_escape_string($con,$_POST['buyingrate']);
        $sellingrate=mysqli_real_escape_string($con,$_POST['sellingrate']);
        $suplier=mysqli_real_escape_string($con,$_POST['suplier']);
        ...
        $previous = $db->queryUniqueValue($con,"SELECT quantity FROM stock_entries WHERE stock_id='$sid'");
        $opening_stock = $db->queryUniqueValue($con,"SELECT quantity FROM stock_entries WHERE stock_id='$sid'");
        if($quantity < $previous)
        {
            $difference = $previous - $quantity;
            $total = $total - $difference;
            $result=$db->query($con,"SELECT * FROM stock_entries where id >= $pid");
        ...
        $db->execute($con,"UPDATE stock_entries SET stock_name='$name',stock_supplier_name='$suplier',category='$category',quantity=$quantity,company_price=$buyingrate,selling_price=$sellingrate,date='$mysqldate',username='$username' WHERE stock_id='$sid'");
        echo "<br><font color=green size=+1 > [$name] Stock Entry Updated !</font>" ;
    }
    if(isset($_GET['id']))
    {
        $id=$_GET['id'];
        $line = $db->queryUniqueObject($con,"SELECT * FROM stock_entries WHERE id=$id");
        ?>
                                    <div class="col-lg-6 col-md-offset-3">
                                        <form role="form" name="form1" method="post" id="form1" action="">
                                            ...
                                            <div class="form-group">
                                                ...
                                                $availquantity = $db->queryUniqueValue($con,"SELECT quantity FROM stock_avail WHERE name='".$line->stock_name."'");
