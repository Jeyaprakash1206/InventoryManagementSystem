							...
							<div class="panel-body">
								<div class="row">
...

									{
										...
										$temp_balance = $db->queryUniqueValue($con,"SELECT balance FROM supplier_details WHERE supplier_name='$supplier'");
										$temp_balance = (int) $temp_balance + (int) $balance;
										$db->execute($con,"UPDATE supplier_details SET balance=$temp_balance WHERE supplier_name='$supplier'");
										...
										foreach($namet as $name1)
										{
											...
											$total=$_POST['total'][$i];
											$sysid=$_POST['sysid'][$i];
											$count = $db->countOf($con,"stock_avail", "name='$name1'");
											if($count == 0)
											{
												$db->query($con,"insert into stock_avail(name,quantity) values('$name1',$quantity)");
												echo "<br><font color=green size=+1 >New Stock Entry Inserted !</font>" ;
												$db->query($con,"insert into stock_details(stock_id,stock_name,stock_quatity,supplier_id,company_price,selling_price) values('$autoid','$name1',0,'$suplier',$brate,$srate)");
												$db->query($con,"UPDATE stock_entries SET stock_name='$name1', stock_supplier_name='$supplier', quantity=$quantity, company_price=$brate, selling_price=$srate, opening_stock=0, closing_stock=$quantity, date='$mysqldate', username='$username', type='entry', total=$total, payment=$payment, balance=$balance, mode='$mode', description='$description', due='$due', subtotal=$subtotal,billnumber='$billnumber' WHERE stock_id='$autoid' AND count1=$j");

											}
											else if($count==1)
											{
												$amount = $db->queryUniqueValue($con,"SELECT quantity FROM stock_avail WHERE name='$name1'");
												$oldquantity = $db->queryUniqueValue($con,"SELECT quantity FROM stock_entries WHERE stock_id='$autoid' AND count1=$j");
												$amount1 = ($amount + $quantity) - $oldquantity;
												if($quantity > $oldquantity)
												{
													$difference = $quantity - $oldquantity;
													$result = $db->query($con,"SELECT opening_stock,closing_stock FROM stock_entries WHERE id > $sysid");
													while ($line10 = $db->fetchNextObject($result)) {
														...
														$db->execute($con,"UPDATE stock_entries SET opening_stock=$new_opening_stock,closing_stock=$new_closing_stock WHERE id=$line10->id");
													}
												}
												if($quantity < $oldquantity)
												{
												...
												$db->execute($con,"UPDATE stock_avail SET quantity=$amount1 WHERE name='$name1'");
								...
								<?php
									...
									{
									$id=$_GET['id'];
									$line = $db->queryUniqueObject($con,"SELECT * FROM stock_entries WHERE stock_id='$id'");
									?>
									...
									<div class="col-lg-10 col-md-offset-2">
											...
											<table   border="0" cellspacing="0" cellpadding="0"  id="dynamictable">
												...
												<tr>
													<div class="form-group-md" align="center">
														...
														<td><textarea class="form-control" name="address1" id="address" style="width:100px;"><?php echo $db->queryUniqueValue($con,"SELECT supplier_address FROM supplier_details WHERE supplier_name='$line->stock_supplier_name'"); ?></textarea></td>
														<td>&nbsp;</td>
														<td>
														...
														<td><input name="contact1" class="form-control" type="text" id="contact1"  value="<?php echo $db->queryUniqueValue($con,"SELECT supplier_contact1 FROM supplier_details WHERE supplier_name='$line->stock_supplier_name'"); ?>" style="width:100px;">
															<br>
															<input name="contact2" class="form-control" type="text" id="contact2"  value="<?php echo $db->queryUniqueValue($con,"SELECT supplier_contact2 FROM supplier_details WHERE supplier_name='$line->stock_supplier_name'"); ?>" style="width:100px;" >
														</td>
													</div>
												</tr>
												<tr>
										...
										<?php
										$max = $db->maxOf($con,"count1", "stock_entries", "stock_id='$id'");
										$j=0;
										for($i=1; $i<=$max; $i++)
										{
												...
												<tr>
													<div class="form-group-md" align="right">
														...
														<td ><input name="name[]" class="form-control" type="text"  id="0<?php echo 0+$j;?>"   style="width:120px;" onFocus="callAutoComplete(this.id)"  onBlur="callAutoAsignValue(this.id)" autocomplete="off" value="<?php
															$line1 = $db->queryUniqueObject($con,"SELECT * FROM stock_entries WHERE stock_id='$id' and count1=$i");
														...
														<td><input name="avail[]" class="form-control" type="text" id="00000<?php echo 4+$j;?>" readonly="" value="<?php echo $db->queryUniqueValue($con,"SELECT quantity FROM stock_avail WHERE name='$line1->stock_name'");?>" style="width:50px;" ></td>
