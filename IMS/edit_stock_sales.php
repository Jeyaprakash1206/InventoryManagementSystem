                            ...
                            <div class="panel-body">
                                <div class="row">
                                    ...
                                    if(isset($_POST['billnumber']))
                                    {
                                        ...
                                        $autoid=mysqli_real_escape_string($con,$_POST['id']);
                                        $customer=mysqli_real_escape_string($con,$_POST['customer1']);
                                        ...
                                        $newvalue =$balance;
                                        $oldvalue = $db->queryUniqueValue($con,"SELECT balance FROM customer_details WHERE customer_name='$customer'");
                                        $diff=$newvalue-$oldvalue;
                                        $temp_balance = (int) $temp_balance +  (int) $diff;
                                        $db->execute($con,"UPDATE customer_details SET balance=$temp_balance WHERE customer_name='$customer'");
                                        ...
                                        foreach($namet as $name1)
                                        {
                                            ...
                                            $mysqldate = date( 'Y-m-d H:i:s', $selected_date );
                                            $username = $_SESSION['username'];
                                            $count = $db->queryUniqueValue($con,"SELECT count(*) FROM stock_avail WHERE name='$name1' and quantity >=$quantity");
                                            if($count == 1)
                                            {
                                                $old_quantity= $db->queryUniqueValue($con,"SELECT quantity FROM stock_sales WHERE transactionid='$autoid' and count1=$i");
                                                $db->query($con,"update stock_sales set  stock_name='$name1',selling_price=$rate,quantity=$quantity,amount=$total,date='$mysqldate',username='$username',customer_id='$customer',subtotal=$subtotal,payment=$payment,balance=$balance,due='$due',mode='$mode',description='$description',billnumber='$billnumber' where transactionid='$autoid' and count1=$j");
                                                $quantity_diff=$quantity-$old_quantity;
                                                $quantity=$quantity+$quantity_diff;
                                                $amount = $db->queryUniqueValue($con,"SELECT quantity FROM stock_avail WHERE name='$name1'");
                                                $amount1 = $amount - $quantity;
                                                $db->query($con,"update stock_entries set stock_id='$autoid',stock_name='$name1',quantity=$quantity,opening_stock=$amount,closing_stock=$amount1,date='$mysqldate',username='$username',type='sales',salesid='$billnumber',total=$total,selling_price=$rate,billnumber='$billnumber' where salesid='$autoid' and count1=$j");
                                       ...
                                       // echo "<div style='background-color:yellow;'><br><font color=green size=+1 >Sales Updated ! Transaction ID [ $autoid ]</font></div> ";
                                        ...
                                        $count1 = $db->queryUniqueValue($con,"SELECT count(*) FROM customer_details WHERE customer_name='$customer'");
                                        if($count1!=1)
                                        {
                                            if($db->query($con,"insert into customer_details values(NULL,'$customer','$address','$contact1','$contact2')"))
                                    ...
                                    {
                                        $id=$_GET['id'];
                                        $line = $db->queryUniqueObject($con,"SELECT * FROM stock_sales WHERE transactionid='$id'");
                                        ?>
                                    <div class="col-lg-10 col-md-offset-2">
                                        <form role="form" name="salesform" method="post" id="form1" action="" onSubmit="updateSubtotal()">
                                            <table   border="0" cellspacing="0" cellpadding="0"  id="dynamictable">
                                                <tr>
                                                    <div class="form-group-md" align="right">
                                                        ...
                                                        <?php
                                                        $max = $db->maxOfAll($con,"id","stock_sales");
                                                        $max=$max+1;
                                                        $autoid="SA".$max."";
                                                        ?>
                                                ...
                                                <tr>
                                                    <div class="form-group-md" align="center">
                                                        ...
                                                        <td><textarea class="form-control" name="address1" id="address" style="width:100px;"><?php echo $db->queryUniqueValue($con,"SELECT customer_address FROM customer_details WHERE customer_name='$line->customer_id'"); ?></textarea></td>
                                                        <td>&nbsp;</td>
                                                        <td>
                                                        ...
                                                        <td><input class="form-control" name="contact1" type="text" id="contact1"  value="<?php echo $db->queryUniqueValue($con,"SELECT 	customer_contact1 FROM customer_details WHERE customer_name='$line->customer_id'"); ?>" style="width:90px;">
                                                            <br>
                                                            <input class="form-control" name="contact2" type="text" id="contact2"  value="<?php echo $db->queryUniqueValue($con,"SELECT 	customer_contact2 FROM customer_details WHERE customer_name='$line->customer_id'"); ?>" style="width:90px;" >
                                                        </td>
                                                    </div>
                                                </tr>
                                                <tr>
                                            ...
                                            <table  border="0" cellspacing="0" cellpadding="0"  id="duplicate" style="">
                                                ...
                                                <?php
                                                $max = $db->maxOf($con,"count1", "stock_sales", "transactionid='$id'");
                                                $j=0;
                                                for($i=1; $i<=$max; $i++)
                                                {
                                                    ?>
                                                ...
                                                <tr>
                                                    <div class="form-group-md" align="right">
                                                        ...
                                                        <td ><input name="name[]" class="form-control" type="text"  id="0<?php echo 0+$j;?>" style="width:120px;" onFocus="callAutoComplete(this.id)"
                                                            ...
                                                            echo $db->queryUniqueValue($con,"SELECT stock_name FROM stock_sales WHERE transactionid='$id' and count1=$i"); ?>">
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            ...
                                                            echo $db->queryUniqueValue($con,"SELECT quantity FROM stock_sales WHERE transactionid='$id' and count1=$i");
                                                            ?>">
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            ...
                                                            echo $db->queryUniqueValue($con,"SELECT selling_price FROM stock_sales WHERE transactionid='$id' and count1=$i");
                                                            ?>"   >
                                                ...
                                                <div class="form-group-md">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><input type="text" class="form-control" name="payment" style="width:100px; " id="payment" class="validate[required,custom[onlyFloat],lengthCheck[6]] text-input" onKeyUp="balanceCalc()"value="<?php
                                                            echo $db->queryUniqueValue($con,"SELECT payment FROM stock_sales WHERE transactionid='$id'");
                                                            ?>"></td>
                                                        <td><div align="left"><strong>Description</strong></div></td>
                                                        <td rowspan="2"><textarea class="form-control" name="description" style="width:150px; height:70px; "><?php echo $db->queryUniqueValue($con,"SELECT description FROM stock_sales WHERE transactionid='$id'"); ?></textarea></td>
                                                        <td><strong>Sub Total </strong></td>
                                                        <td>&nbsp;</td>
                                                        <td><input name="subtotal" class="form-control" id="subtotal" type="text" value="<?php
                                                            echo $db->queryUniqueValue($con,"SELECT subtotal FROM stock_sales WHERE transactionid='$id'");?>" readonly="" style="width:100px; text-align:right; color:#333333; font-weight:bold; font-size:16px;"></td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td><input name="balance" class="form-control" type="text" id="balance" style="width:100px; " value="<?php
                                                            echo $db->queryUniqueValue($con,"SELECT balance FROM stock_sales WHERE transactionid='$id'");
                                                            ?>" readonly=""></td>
                                                        <td>&nbsp;</td>
                                                        <td>&nbsp;</td>
