...
else
{
	...
	$result =$db->query($con,"SELECT * FROM stock_entries WHERE stock_id='$id'");
  while ($line = $db->fetchNextObject($result)) {
			
				$difference=$db->queryUniqueValue($con,"SELECT quantity FROM stock_entries WHERE id=$line->id");
				
				$name=$db->queryUniqueValue($con,"SELECT stock_name FROM stock_entries WHERE id=$line->id");
				$result1=$db->query($con,"SELECT * FROM stock_entries where id > $line->id");
				...
				$db->execute($con,"DELETE FROM stock_entries WHERE id=$line->id");
